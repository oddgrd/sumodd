...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  // TODO: move to init function, maybe include statemachine init, e.g. app_init().
  ir_remote_init();
  line_sensor_init();
  vl53lox_init();
  motor_driver_init();
...
void SystemClock_Config(void)
{
  ...
  PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_I2C1 | RCC_PERIPHCLK_TIM1 | RCC_PERIPHCLK_ADC12;
